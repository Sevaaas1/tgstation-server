...
namespace TGS.ControlPanel
{
	...
	{
		public static IServerClient ServerInterface;
		[STAThread]
		static void Main(string[] args)
		{
			try
			{
				...
				{
					new InstanceSelector(ServerInterface).Show();
					Application.Run();
				}
			}
			catch (Exception e)
		...
		public static bool GetRepositoryRemote(IRepositoryClient repoClient, out string owner, out string name)
		{
			var repo = repoClient.Read(default).GetAwaiter().GetResult();
			owner = repo.RemoteRepositoryOwner;
			name = repo.RemoteRepositoryName;
			return repo.RemoteGitProvider != Tgstation.Server.Api.Models.RemoteGitProvider.Unknown;
		}
	}
}
